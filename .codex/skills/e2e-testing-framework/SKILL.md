...
description: E2E testing patterns using Chrome DevTools MCP. Use when writing E2E tests, browser automation, or any test requiring Step 0 auth and step-by-step execution.
---
...
2. **Test Credentials Setup**
   - Copy `dev/test-credentials.example.json` to `dev/test-credentials.json` and fill in test accounts (NOT committed to version control), or use `.env.test` with `TEST_USER_EMAIL`, `TEST_USER_PASSWORD`, etc.
